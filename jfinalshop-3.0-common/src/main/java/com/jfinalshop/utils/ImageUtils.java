...
public final class ImageUtils {
	...
	public static void zoom(File srcFile, File destFile, int destWidth, int destHeight) {
		AssertUtil.notNull(srcFile);
		if (srcFile == null) {
			throw new IllegalArgumentException("srcFile为空");
		}
		AssertUtil.notNull(destFile);
		AssertUtil.state(destWidth > 0);
		AssertUtil.state(destHeight > 0);
		if (type == Type.jdk) {
	...
	public static void addWatermark(File srcFile, File destFile, File watermarkFile, WatermarkPosition watermarkPosition, int alpha) {
		AssertUtil.notNull(srcFile);
		AssertUtil.notNull(destFile);
		AssertUtil.state(alpha >= 0);
		AssertUtil.state(alpha <= 100);

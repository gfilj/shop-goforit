...
import com.jfinalshop.utils.ReturnKit;
 ...
 * <p>
 * <p>
 * This filter constructs a {@link org.apache.shiro.authc.UsernamePasswordToken
 * UsernamePasswordToken} with the values found in
 * </p>
 * {@link #setUsernameParam(String) username}, {@link #setPasswordParam(String)
 * password}, and {@link #setRememberMeParam(String) rememberMe} request
 * parameters. It then calls
 ...
 * <code>AuthenticationException</code> fully qualified class name will
 * </p>
 * be set as a request attribute under the
 * {@link #setFailureKeyAttribute(String) failureKeyAttribute} key. This FQCN
 * can be used as an i18n key or lookup mechanism to explain to the user why
 * their login attempt failed (e.g. no user, incorrect password, etc).
 * <p>
 * <p>
 * If you would prefer to handle the authentication validation and login in your
 * own code, consider using the
 * </p>
 * {@link org.apache.shiro.web.filter.authc.PassThruAuthenticationFilter}
 * instead, which allows requests to the {@link #loginUrl} to pass through to
 * your application's code directly.
 *
...
public class ShiroFormAuthenticationFilter extends ShiroAuthenticatingFilter {
	...
	protected void clearFailureAttribute(ServletRequest request, ServletResponse response, boolean returnJson) {
		if (returnJson) {
			request.setAttribute("admin", SubjectKit.getAdmin());

...
import com.jfinalshop.plugin.shiro.exception.IncorrectCaptchaException;
import com.jfinalshop.utils.EncriptionKit;


/**
 * An <code>AuthenticationFilter</code> that is capable of automatically
 * performing an authentication attempt based on the incoming request.
 *
 * @since 0.9
 */
public abstract class ShiroAuthenticatingFilter extends
	...
	protected void doCaptchaValidate(CaptchaUsernamePasswordToken token) {
		...
		if (session == null) {
			...
			if (session.getAttribute(DEFAULT_CAPTCHA_PARAM) != null) {
				...
				if (token.getCaptcha() != null && captcha.equalsIgnoreCase(EncriptionKit.encrypt(token.getCaptcha().toLowerCase()))) {
					return;
				}
			}
